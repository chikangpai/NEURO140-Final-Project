...
class Adapter(nn.Module):
    ...
    def forward(self, x):
        adapter_out = self.up_proj(self.nonlin(self.down_proj(x)))
        return x + adapter_out


class ClfNet(nn.Module):
    def __init__(self, featureLength=768, classes=2, dropout=None, adapter_type=None, adapter_dim=64):
        super(ClfNet, self).__init__()
        self.featureExtractor = MILNet(featureLength=featureLength, dropout=dropout)
        
        # Add adapter if specified
        if adapter_type is not None and adapter_type != "":
            self.adapter = Adapter(featureLength, adapter_dim)
        else:
            self.adapter = None
